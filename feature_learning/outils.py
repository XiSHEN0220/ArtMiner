...
## The minimum dimensions in the feature map is at least query_feat_min

def resize_dim_image(query_feat_min, feat_max, stride_net, w, h) :
	
	ratio = float(w)/h
	if ratio < 1 : 
		feat_h = feat_max
		feat_w = max(round(ratio * feat_h), query_feat_min)
	else : 
		feat_w = feat_max
		feat_h = max(round(feat_w/ratio), query_feat_min)
		
	resize_w, resize_h = int(feat_w * stride_net), int(feat_h * stride_net)
...
def SearchImgFeat(searchDir, margin, searchRegion, scales, strideNet, transform, model, searchName, useGpu) :
	...
	for s in scales : 
		...
		new_w, new_h = resize_dim_image(2 * margin + searchRegion + 1, s, strideNet, w, h)
...
def RetrievalRes(nbPatchTotal, imgList, searchDir, margin, searchRegion, scales, strideNet, transform, net, featQuery, useGpu) : 
	...
	for i in tqdm(range(len(imgList))) :
		...
		searchFeat = SearchImgFeat(searchDir, margin, searchRegion, scales, strideNet, transform, net, search_name, useGpu)
...
def VotePair(searchDir, imgList, topkImg, topkScale, topkW, topkH, transform, net, margin, validRegion, searchRegion, featChannel, useGpu, strideNet) :
	...
	w,h = I1.size
	new_w, new_h = resize_dim_image(2 * margin + searchRegion + 1, info1[1], strideNet, w, h)
	...
	w,h = I2.size
	new_w, new_h = resize_dim_image(2 * margin + searchRegion + 1, info2[1], strideNet, w, h)
...
def PosCosineSimilaritytop1(feat1, feat2, pos_w1, pos_h1, pos_w2, pos_h2, variableAllOne) :
	...
	return CosineSimilarityTopK(featRec, featRecNorm, feat1x1.unsqueeze(2).unsqueeze(3), K = 1)[0]
...
def PosNegaSimilarity(posPair, posIndex, topkImg, topkScale, topkW, topkH, searchDir, imgList, strideNet, net, transform, searchRegion, trainRegion, margin, featChannel, useGpu, topKLoss) : 
	...
	I1 = Image.open(os.path.join(searchDir, imgList[info1[0]])).convert('RGB')
	w,h = I1.size
	new_w, new_h = resize_dim_image(margin * 2 + searchRegion + 1, info1[1], strideNet, w, h)
	...
	I2 = Image.open(os.path.join(searchDir, imgList[info2[0]])).convert('RGB')
	w,h = I2.size
	new_w, new_h = resize_dim_image(margin * 2 + searchRegion + 1, info2[1], strideNet, w, h)
